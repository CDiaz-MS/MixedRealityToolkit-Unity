...
namespace Microsoft.MixedReality.Toolkit.UI.Interaction
{
    ...
    ///
    ///</summary>
    public class InteractivePressableButton : InteractiveButton
    {
                ...
                if (value != isTouching)
                {
                    ...
                    if (!isTouching)
                    {
                        ...
                        if (!releaseOnTouchEnd)
                        {
                            ...
                            if (IsStateTracking("Press"))
                            {
                                SetStateOff(CoreInteractionState.Press);

                                EventReceiverManager.InvokeStateEvent("Press", null);
                            }
                        }
                    }
                }
        ...
        protected virtual void Start()
        {
            ...
            {
                var touchEventConfig = EventReceiverManager.GetEventConfiguration("Touch") as TouchInteractionEventConfiguration;

                touchEventConfig.OnTouchStarted.AddListener((eventData) =>
                {
                ...
                touchEventConfig.OnTouchCompleted.AddListener((eventData) =>
                {
                    if (touchPoints.ContainsKey(eventData.Controller))
                    {
                ...
                touchEventConfig.OnTouchUpdated.AddListener((eventData) =>
                {
                    if (touchPoints.ContainsKey(eventData.Controller))
                    {
                        touchPoints[eventData.Controller] = eventData.InputData;
        ...
        private void UpdatePressedState(float pushDistance)
        {
            ...
            if (!IsPressing)
            {
                ...
                if (pushDistance >= pressDistance)
                {
                    ...
                    if (IsStateTracking("Press"))
                    {
                        SetStateOn(CoreInteractionState.Press);
