...
namespace Microsoft.MixedReality.Toolkit.UI.Layout
{
    public class UIVolumeFlex : UIVolume
    {
        ...
        public void RowDistribution(VolumeAxis axis, List<UIVolume> childrenToDistribute, bool YSpace = false)
        {
            ...
            foreach (UIVolume child in childrenToDistribute)
            {
        ...
        private Vector3 GetAxisSurfaceArea(List<UIVolume> surfaceAreaList)
        {
            List<Vector3> axisSizes = new List<Vector3>();

            foreach (UIVolume childVolume in surfaceAreaList)
            {
